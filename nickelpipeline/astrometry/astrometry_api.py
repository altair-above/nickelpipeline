...
#         output_path_stem = image_path_path.stem.split('_')[0]
#         if mode == 'image':
#             output_path = os.path.join(output_dir, output_path_stem + '_astro' + '.fits')
#         elif mode == 'corr':
#             output_path = os.path.join(output_dir, output_path_stem + '_astro' + '_corr' + '.fits')
#         if not resolve and os.path.exists(output_path):
#             logger.info(f"Returning local copy of {image_path_path.name}'s solution; astrometry.net not used")
...
#                 logger.warning(f"***Connection Error encountered; skipping image {image_path_path.name} & waiting 15 sec***")
#                 time.sleep(15)
