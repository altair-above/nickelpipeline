...
def photometric_calib_all(astrophot_dir):
    ...
    for obj_dir in astrophot_dir.iterdir():
        ...
        if '109_199' in obj_dir.name:
            ...
            stdrd_coords = [(280.2180417, 0.0318750), (280.1902500, 0.0304917),
                            (280.219625, 0.0141222), (280.2145417, 0.0397944)]
            stdrd_mags = [12.52, 13.65, 12.77, 14.28]
            stdrd_names = ['110_232', '110_229', '110_233', '110_230']
...
def fit_zk(stdrd_coords, stdrd_mags, phot_data_paths, flux_name='flux_psf'):
    ...
    for coord, m_a in zip(stdrd_coords, stdrd_mags):
        logger.info(f"Fitting z, k to mag {m_a:.3f} standard star at {coord}")
        
        m_is = []
        airmasses = []
        for phot_data_path in phot_data_paths:
            phot_data = ascii.read(phot_data_path, format='csv')
            # print(phot_data['coord_obj'][0], type(phot_data['coord_obj'][0]))
            # ra = [coord[0] for coord in phot_data['coord_obj']]
            # dec = [coord.dec for coord in phot_data['coord_obj']]
            world_coords = [(ra, dec) 
...
def format_table(phot_data):
    colnames = ['group_id', 'group_size', 'flags', 'ra_hms', 'dec_dms',
                ...
                'x_fit', 'y_fit', 'ra_deg', 'dec_deg', 'x_err', 'y_err', 
