...
#     masked_images = np.ma.array([image.masked_array_cols_only for image in images])
...
#     mean_rkron = np.zeros(num_images, dtype=float)
#     source_data = np.zeros((0, 9), dtype=float)
#     all_stamps = np.empty(num_images, dtype=object)
...
#         ap_centers = np.column_stack((sources['xcentroid'], sources['ycentroid']))
...
#         center = np.atleast_1d(stamp_shape)/2
#         stamps = np.ma.zeros((nsources,)+stamp_shape, dtype=float)
#         rkron = np.zeros(nsources, dtype=float)
#         kflux_raw = np.zeros(nsources, dtype=float)
#         bkg = np.zeros(nsources, dtype=float)

#         # Background subtract and other things??
#         for i in range(nsources):
#             t = transform.EuclideanTransform(translation=-ap_centers[i][::-1]-0.5 + center)
#             stamps[i] = transform.warp(masked_images[index].filled(0.0).T, t.inverse, output_shape=stamp_shape,
#                                        cval=0, order=0).T
#             stamps[i,stamps[i] == 0.] = np.ma.masked
...
#             if not np.any(indx):
#                 continue
#             kflux_raw[i] = np.ma.sum(stamps[i,indx])
...
#         _kflux = np.ma.MaskedArray(np.tile(kflux, (nsources,1)),
#                                       mask=np.logical_not(too_close))
#         keep = (np.ma.argmax(_kflux, axis=1) == np.arange(nsources)) & (kflux > 1e3)#& (kflux > 1e3)
    
#         keep &= (np.sum(stamps.mask, axis=(1,2)) == 0)
#         nkeep = np.sum(keep)
#         if nkeep == 0:
...
#         if verbose: print(f'Working on image {images[index]} ({len(stamps[keep])} stamps)')
