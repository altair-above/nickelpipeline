...
def reduce_all(rawdir):
    ...
    logger.info(f"Performing overscan subtraction on {len(rawfiles)} images")
    ...
    logger.info(f"Performing bias subtraction on {len(df_log.file)-len(biasfiles)} non-bias files")
    logger.info(f"Using {len(biasfiles)} bias frames: {[file.name.split('_')[0] for file in biasfiles]}")
    ...
    for sat_col in sat_columns:
        bias[:,sat_col] = 0
        logger.debug(f"Setting saturated column {sat_col} = 0")
    ...
    logger.info(f"Bias subtracted images saved to {nonbias_dir}")
    ...
    for flatfilt in flatfilts:
        ...
        logger.info(f"{flatfilt} Filter - Using {len(flatfiles)} flats: {[file.name.split('_')[0] for file in flatfiles]}")
        ...
        logger.info(f"{flatfilt} Filter - Running flat division on {len(scienceobjects)} science objects")
        ...
        if len(scienceobjects) > 0:
            for scienceobject in scienceobjects:
                ...
                if len(sciencefiles) > 0:
                    logger.info(f"{flatfilt} Filter - Saving {len(sciencefiles)} fully reduced {scienceobject} images to {sci_dir}")
