...
def process_multiple(file_list, bias, flats):
    images = unzip_directories(file_list, output_format='Path')
    # if not isinstance(images):
    #     images = [Path(rawfiles)]
    # else:
    #     images = [Path(file) for file in rawfiles]
    
    red_files = [process_single(image, bias, flats) for image in images]
