...
#                                         ) #, verbose=2, jac=self.deriv_resid)
#         new_par = result.x
#         if 0.92 < new_par[3]/new_par[4] < 1.08:
#             new_par[5] = 0
...
#         """Convert phi to angle between semi-major axis & +x-axis (degrees)

#         Args:
#             par (list): Fit parameters (from FitEllipticalMoffat2D self.par)

#         Returns:
#             float: Nice phi (degrees)
#         """
...
#         Returns:
#             ndarray: Derivative of the residuals.
#         """
#         self._set_par(par)
#         dmoff = self.moff.fit_deriv(self.x.ravel(), self.y.ravel(), self.par[2], self.par[0],
#                                     self.par[1], self.par[3], self.par[4])
...
#     Parameters are:
#         x0: x-coordinate of the cennter
#         y0: y-coordinate of the center
#         amplitude: 
#         gamma1: "width" along the one axis
#         gamma2: "width" along the other axis
#         phi: rotation angle
#         alpha: fall-off slope
...
#     def model(self, par=None):
#         """
#         Evaluate the elliptical Moffat model with the object's parameters 
#         or given parameters.
...
#         if par is not None:
#             self._set_par(par)
#         _x = (self.x-self.par[0])*numpy.cos(self.par[5]) - (self.y-self.par[1])*numpy.sin(self.par[5])
#         _y = (self.x-self.par[0])*numpy.sin(self.par[5]) + (self.y-self.par[1])*numpy.cos(self.par[5])
#         return self.par[2]*numpy.power(1 + (_x/self.par[3])**2 + (_y/self.par[4])**2, -self.par[6]) + self.par[7]
...
#         Args:
#             par (ndarray): Parameters for the model.
#                            (x0, y0, amplitude, gamma1, gamma2, phi, alpha, background)
...
#     def fit(self, p0=None, fix=None, lb=None, ub=None):
#         """
#         Fit the elliptical Moffat model to the data and modify parameters accordingly.
